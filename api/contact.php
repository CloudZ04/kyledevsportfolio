...
// Vercel: PHP runs from api/, project root is one level up
...
// Optional backup log (may be read-only on serverless)
$logPath = $projectRoot . '/submissions.txt';
...
try {
    ...
    // Logo: embed PNG if present (path from project root)
    $logoHtml = "<span style=\"font-weight:700;color:#feffb0;font-size:18px;\">Kyle Devs</span>";
    $emailLogoPath = $projectRoot . '/assets/email-logo.png';
    ...
    $errPath = $projectRoot . '/email_errors.log';
    if (is_writable($projectRoot)) {
        @file_put_contents($errPath, date('Y-m-d H:i:s') . " - " . $mail->ErrorInfo . "\n", FILE_APPEND);
    }
    sendResponse(false, $isAjax);
}
